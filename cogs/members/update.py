...
class UpdateMember(commands.Cog):
    ...
    async def on_member_update(self, before: disnake.Member, after: disnake.Member):
        ...
        if before.nick != after.nick:
            changes.append(f"Никнейм изменён с {before.nick or 'None'} на {after.nick or 'None'}")
        ...
        if before.roles != after.roles:
            ...
            if removed_roles:
                changes.append(f"Роли удалены: {', '.join(role.mention for role in removed_roles)}")
            if added_roles:
                changes.append(f"Роли добавлены: {', '.join(role.mention for role in added_roles)}")
