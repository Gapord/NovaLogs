...
class CreateEmoji(commands.Cog):
    ...
    async def on_guild_emojis_update(self, guild: disnake.Guild, before: list[disnake.Emoji], after: list[disnake.Emoji]):
        ...
        for emoji in new_emojis:
            async for entry in guild.audit_logs(action=disnake.AuditLogAction.emoji_create, limit=1):
                creator = entry.user
                break
            else:
                creator = None
            ...
            embed = disnake.Embed(title="Создание эмодзи", color=await getcolor(guild.id))
            embed.add_field(name="Эмодзи", value=f"{emoji} (ID: {emoji.id})")
            embed.add_field(name="Создано пользователем", value=creator.mention if creator else "Неизвестен")
