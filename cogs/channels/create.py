...
class CreateChannel(commands.Cog):
    ...
    async def on_guild_channel_create(self, channel: disnake.abc.GuildChannel):
        ...
        async for entry in guild.audit_logs(action=disnake.AuditLogAction.channel_create, limit=1):
            creator = entry.user
            break
        else:
            creator = None

        channel_type = "Категория" if isinstance(channel, disnake.CategoryChannel) else \
                       "Текстовый канал" if isinstance(channel, disnake.TextChannel) else \
                       "Голосовой канал" if isinstance(channel, disnake.VoiceChannel) else \
                       "Канал"
        ...
        embed.add_field(name="Ответственный", value=creator.mention if creator else "Неизвестен")
