...
class DeleteChannel(commands.Cog):
    ...
    async def on_guild_channel_delete(self, channel: disnake.abc.GuildChannel):
        ...
        async for entry in guild.audit_logs(action=disnake.AuditLogAction.channel_delete, limit=1):
            deleter = entry.user
            break
        else:
            deleter = None
        ...
        embed.add_field(name="Ответственный", value=deleter.mention if deleter else "Неизвестен")
