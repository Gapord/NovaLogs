...
class UpdateRole(commands.Cog):
    ...
    async def on_guild_role_update(self, before: disnake.Role, after: disnake.Role):
        ...
        async for entry in guild.audit_logs(action=disnake.AuditLogAction.role_update, limit=1):
            updater = entry.user
            break
        else:
            updater = None
        ...
        if before.hoist != after.hoist:
            changes.append(f"Поднятие роли изменено на `{'Включено' if after.hoist else 'Выключено'}`")

        if before.mentionable != after.mentionable:
            changes.append(f"Упоминание роли изменено на `{'Включено' if after.mentionable else 'Выключено'}`")
        ...
        embed.add_field(name="Обновлено пользователем", value=updater.mention if updater else "Неизвестен")
