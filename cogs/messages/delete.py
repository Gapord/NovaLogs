...
class DeleteMSG(commands.Cog):
    ...
    async def on_message_delete(self, message):
        async for entry in message.guild.audit_logs(action=disnake.AuditLogAction.message_delete, limit=1):
        ...
        embed = disnake.Embed(title="Удаление сообщения", color=await getcolor(message.guild.id))
