...
class UpdateChannel(commands.Cog):
    ...
    async def on_guild_channel_update(
        ...
        async for entry in after.guild.audit_logs(
            action=disnake.AuditLogAction.channel_update, limit=1
        ):
            updater = entry.user
            break
        else:
        ...
        if before.category != after.category:
            changes.append(
                f"Категория изменена с `{before.category.name if before.category else 'Нет'}` на `{after.category.name if after.category else 'Нет'}`"
            )

        if before.position != after.position:
            changes.append(
                f"Позиция изменена с `{before.position}` на `{after.position}`"
            )

        if isinstance(before, disnake.TextChannel) and isinstance(
            after, disnake.TextChannel
        ):
            if before.topic != after.topic:
                changes.append(
                    f"Описание изменено с `{before.topic or 'Нет'}` на `{after.topic or 'Нет'}`"
                )

            if before.slowmode_delay != after.slowmode_delay:
                changes.append(
                    f"Slowmode изменён с `{before.slowmode_delay} секунд` на `{after.slowmode_delay} секунд`"
                )

            if before.nsfw != after.nsfw:
                changes.append(
                    f"NSFW-режим изменён на `{'Включен' if after.nsfw else 'Выключен'}`"
                )
